...
class MotifBackground extends StatelessWidget {
  const MotifBackground({Key? key}) : super(key: key);
...
class myPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    Paint paint = new Paint()
      ...
      ..isAntiAlias = true
      ..color = Color(0xff9AE8E1).withOpacity(0.5);
    Offset circleCenter = Offset(size.width / 2, size.height - 160 * 0.28);
    Offset topLeft = Offset(0, 0);
    Offset bottomLeft = Offset(0, 180 * 0.30);
    Offset topRight = Offset(size.width, 0);
    Offset bottomRight = Offset(size.width, 30);
